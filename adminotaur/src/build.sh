#!/bin/bash
# Adminotaur Agent PyInstaller Build Script (Linux) via Poetry

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
ENTRYPOINT="$SCRIPT_DIR/adminotaur.py"
OUTPUT_DIR="$PROJECT_ROOT"
BUILD_DIR="$SCRIPT_DIR/.build"

echo "==> Building Adminotaur PyInstaller binary using Poetry"
...
# Ensure auxiliary files exist alongside binary
touch "$OUTPUT_DIR/adminotaur.json" >/dev/null 2>&1 || true
touch "$OUTPUT_DIR/adminotaur.md" >/dev/null 2>&1 || true

echo "==> Build complete: $OUTPUT_DIR/adminotaur"
